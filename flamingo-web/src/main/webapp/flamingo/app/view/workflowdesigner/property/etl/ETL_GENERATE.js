 ...
 * ETL Generate
 *
 * @cli hadoop jar flamingo2-mapreduce-hadoop2-2.0.5-job.jar generate --input <IN> --output <OUT> --inputDelimiter ',' --outputDelimiter '|' --sequenceIndex 0 --startNumber 1 --columnSize 12 --generateType 'SEQUENCE'
...
Ext.define('Flamingo.view.workflowdesigner.property.etl.ETL_GENERATE', {
    extend: 'Flamingo.view.workflowdesigner.property._NODE_ETL',
    alias: 'widget.ETL_GENERATE',
    ...
    items: [
        {
            ...
            items: [
                ...
                {
                    xtype: 'numberfield',
                    fieldLabel: message.msg('workflow.etl.generate.sequenceIndex'),
                    emptyText: message.msg('workflow.etl.generate.sequenceIndexEmpty'),
                    name: 'sequenceIndex',
                    minValue: 0,
                    value: 0
                },
                {
                    xtype: 'combobox',
                    fieldLabel: message.msg('workflow.etl.generate.generateType'),
                    name: 'generateType',
                    value: 'SEQUENCE',
                    allowBlank: false,
                    editable: false,
                    ...
                    store: {
                        fields: ['displ', 'value'],
                        data: [
                            {displ: 'SEQUENCE', value: 'SEQUENCE'},
                            {displ: 'TIMESTAMP', value: 'TIMESTAMP'}
                        ]
                    }
                },
                {
                    xtype: 'numberfield',
                    fieldLabel: message.msg('workflow.etl.generate.startNumber'),
                    name: 'startNumber',
                    minValue: 0,
                    value: 0
                },
                {
                    xtype: 'textfield',
                    fieldLabel: message.msg('workflow.etl.generate.dateFormat'),
                    name: 'dateFormat',
                    value: 'yyyyMMdd'
                }
            ]
    ...
    afterPropertySet: function (props) {
        ...
        if (props.sequenceIndex) {
            props.mapreduce.params.push('--sequenceIndex', props.sequenceIndex);
        }
        props.mapreduce.params.push('--generateType', props.generateType);
        if (props.generateType === 'SEQUENCE') {
            props.mapreduce.params.push('--startNumber', props.startNumber);
        } else {// TIMESTAMP
            props.mapreduce.params.push('--dateFormat', '\'' + props.dateFormat + '\'');
        }
