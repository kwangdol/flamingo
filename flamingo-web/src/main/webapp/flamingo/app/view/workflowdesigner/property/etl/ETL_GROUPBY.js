 ...
 * ETL Group by
 *
 * @cli hadoop jar flamingo2-mapreduce-hadoop2-2.0.5-job.jar groupby --input <IN> --output <OUT> --inputDelimiter ',' --keyValueDelimiter '|' --groupByKey 12 --allowDuplicate COLUMN --allowSort 0 --valueDelimiter '112\.216\.16\.138'
...
Ext.define('Flamingo.view.workflowdesigner.property.etl.ETL_GROUPBY', {
    extend: 'Flamingo.view.workflowdesigner.property._NODE_ETL',
    alias: 'widget.ETL_GROUPBY',
    ...
    items: [
        {
            ...
            items: [
                ...
                {
                    xtype: 'fieldcontainer',
                    fieldLabel: message.msg('workflow.etl.groupby.valueDelimiter'),
                    layout: 'hbox',
                    items: [
                        {
                            xtype: 'combo',
                ...
                {
                    xtype: 'numberfield',
                    fieldLabel: message.msg('workflow.etl.groupby.groupByKey'),
                    name: 'groupByKey',
                    minValue: 0,
                    allowBlank: false
                },
                {
                    xtype: 'checkbox',
                    fieldLabel: message.msg('workflow.etl.groupby.allowDuplicate'),
                    name: 'allowDuplicate',
                    inputValue: true
                },
                {
                    xtype: 'checkbox',
                    fieldLabel: message.msg('workflow.etl.groupby.allowSort'),
                    name: 'allowSort',
                    inputValue: true
                }
            ]
    ...
    afterPropertySet: function (props) {
        ...
        props.mapreduce.params.push('--keyValueDelimiter', '\'' + (props.keyValueDelimiter === 'CUSTOM' ? props.keyValueDelimiterValue : props.keyValueDelimiter) + '\'');
        props.mapreduce.params.push('--valueDelimiter', '\'' + (props.valueDelimiter === 'CUSTOM' ? props.valueDelimiterValue : props.valueDelimiter) + '\'');
        props.mapreduce.params.push('--groupByKey', props.groupByKey);
        if (props.allowDuplicate) {
            props.mapreduce.params.push('--allowDuplicate', props.allowDuplicate);
        }
        if (props.allowSort) {
            props.mapreduce.params.push('--allowSort', props.allowSort);
        }
